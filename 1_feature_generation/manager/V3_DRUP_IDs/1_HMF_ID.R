...
clin.df = fread('data/20230127_DRUP_pembro_LL_curated.tsv')
...
# 
# #22 missing when using WGS ID
# clin.df$WGS_ID[!clin.df$WGS_ID %in% c(drup.fn, cpct.fn)]
# 
# #Those missing DRUP ID get WGS_ID
# clin.df$patientID[!clin.df$patientID %in% c(drup.fn, cpct.fn)] = NA
# clin.df$patientID[is.na(clin.df$patientID)] = clin.df$WGS_ID[is.na(clin.df$patientID)]
# clin.df$patientID[!clin.df$patientID%in% c(drup.fn, cpct.fn)]
# #same as be using WGS ID, conclusion: we need CPCT samples.

#SO ACTUALLY:
clin.df$patientID = clin.df$WGS_ID
clin.df$patientID[!clin.df$patientID %in% c(drup.fn, cpct.fn)]

clin.df$WGS_available = rep("yes", nrow(clin.df))
clin.df$WGS_available[!clin.df$patientID %in% c(drup.fn, cpct.fn)] = "no"

write.table(clin.df, file='data/20230127_DRUP_pembro_LL_final.tsv', 
